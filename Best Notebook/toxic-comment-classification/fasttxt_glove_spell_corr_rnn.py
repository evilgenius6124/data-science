...
'''remove special char after converting to ascii  and  full empty sent with something'''
...
# For best score (Public: 0.9869, Private: 0.9865), change to max_features = 283759, maxlen = 900
max_features = 10000
maxlen = 50
...
EMBEDDING_FILE_FASTTEXT="C:/Users/Bhawani Singh/Desktop/Toxic Comment Classification Challenge/data/crawl-300d-2M.vec/crawl-300d-2M.vec"
EMBEDDING_FILE_TWITTER="C:/Users/Bhawani Singh/Desktop/Toxic Comment Classification Challenge/data/glove.twitter.27B.200d.txt/glove.twitter.27B.200d"
...
'''word2vec : orders words in decreasing order of freq. in training corpse'''
    ...
    if embeddings_index_ft.get(word) is not None:
        ...
        if len(word) > 0:
            ...
            if embeddings_index_ft.get(word2) is not None:
                ...
                word2 = correction(singlify(word))    #okkkkkk to ok
                if embeddings_index_ft.get(word2) is not None:
                    embed_word(embedding_matrix,i,word2)
                else:
                    embedding_matrix[i] = something     
...
def get_model(features,clipvalue=1.,num_filters=40,dropout=0.5,embed_size=501):
    ...
    x, x_h, x_c = Bidirectional(GRU(num_filters, return_sequences=True, return_state = True))(x)  #return last o/p and state
